...
provider_polygon.addAttributes([QgsField("Descripción", 10)])
...
segments = 36  # Aumentar el número de segmentos para suavizar el polígono
...
combined_polygon.setAttributes(["Polígono combinado de terremotos"])
provider_polygon.addFeature(combined_polygon)

# Añadir la capa de polígonos al proyecto
QgsProject.instance().addMapLayer(polygon_layer)

# Aplicar simbología para el polígono sin relleno y con borde visible
symbol = QgsFillSymbol.createSimple({
    "style": "no",  # Sin relleno
    "outline_color": "red",  # Color del borde
    "outline_width": "1",  # Ancho del borde
})
polygon_layer.renderer().setSymbol(symbol)
polygon_layer.triggerRepaint()

print("Polígonos generados y añadidos al proyecto.")
